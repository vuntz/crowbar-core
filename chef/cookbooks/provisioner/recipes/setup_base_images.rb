...
node[:provisioner][:supported_oses].each do |os, arches|
  arches.each do |arch, params|
    ...
    if File.exists?("#{os_dir}/crowbar-extra") && File.directory?("#{os_dir}/crowbar-extra")
    ...
    when /^(suse)/ =~ os
      ...
      node.set[:provisioner][:repositories][os][arch]["base"] = { "baseurl=http://#{admin_ip}:#{web_port}/#{os}/install" => true }
    ...
    when /^(redhat|centos)/ =~ os
      ...
      if ::File.exists? "#{tftproot}/#{os}/install/repodata"
        ...
        node.set[:provisioner][:repositories][os][arch]["base"] = { "baseurl=http://#{admin_ip}:#{web_port}/#{os}/install/Server" => true }
      end
    ...
    when /^ubuntu/ =~ os
      node.set[:provisioner][:repositories][os][arch]["base"] = { "http://#{admin_ip}:#{web_port}/#{os}/install" => true }
