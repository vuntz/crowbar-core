...
wait_for_pxe() {
    ...
    until [ 1 = $done ] ; do
        if [ -n "$state" ]; then
            curl --fail --silent --connect-timeout 5 "http://$ADMIN_IP:8091/discovery/bios/pxelinux.cfg/$MYHEXIP" | grep -q "^DEFAULT $state$"
            ret=$?
        else
            curl --fail --silent --head --connect-timeout 5 "http://$ADMIN_IP:8091/discovery/bios/pxelinux.cfg/$MYHEXIP" > /dev/null
            ret=$?
        fi
